function inverse_flow_map_1D(f,xrange,trange)
%
...
f=@(t,x) -f(t,x); % right hand side of the ODE
...
zlabel('$X_0(t,x)$','Interpreter','Latex')
set(gca,'Fontsize',20)
colorbar
axis tight
...
clf
hold on
plot(x0,ZZ(find(ts==10),:),'b', 'Linewidth',4)
plot(x0,ZZ(find(ts==20),:),'g--', 'Linewidth',4)
plot(x0,ZZ(find(ts==30),:),'r--', 'Linewidth',3)
plot(x0,ZZ(find(ts==40),:),'y--', 'Linewidth',1)
h=legend('$t=10$','$t=20$','$t=30$','$t=40$');
set(gca,'Fontsize',20)
box
set(h,'Interpreter','Latex','Location','NorthWest')
axis([xrange(1) xrange(end) xrange(1)-1 xrange(end)+1])
axis([xrange(1) xrange(end) -20 20])
xlabel('$x$','Interpreter','Latex')
ylabel('$X_0(t,x)$','Interpreter','Latex')
end